...
bool Application::Update()
{
	...
	for (MissileList::iterator missile = mymissile.begin();
		missile != mymissile.end(); missile++)
	{
		if ((*missile)->Update(ships_, timedelta))
		{
			// have collision
			delete *missile;
			mymissile.erase(missile);
			break;
		}
	}
...
void Application::CreateMissile(float x, float y, float w, int id)
{
	...
	mymissile.push_back(new Missile("missile.png", x, y, w, id));
